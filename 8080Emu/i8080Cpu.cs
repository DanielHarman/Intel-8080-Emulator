...
namespace _8080Emu
{
    ...
    public class i8080Cpu
    {
        public byte[] memory;
        public byte[] iDevices, oDevices;
        ...
        public i8080Cpu()
        {
            ...
            iDevices = new byte[255];
            oDevices = new byte[255];
        ...
        public void execute()
        {
            ...
            Console.WriteLine("SP: 0x{0:X4}", stackPointer);
            ...
            {
                Console.WriteLine("NON IMPLEMENTED OR INCORRECT INSTRUCTION ->{0:X2}<-", instruction);
            }
            if (!jumped)
                programCounter++;
        }
        ...
        public void cpu_in()
        {
            reg_a = iDevices[memory[programCounter + 1]];
        }
        ...
        public void cpu_out()
        {
            oDevices[memory[programCounter + 1]] = reg_a;
        }
        ...
        public void jump()
        {
            ...
            Console.WriteLine("Jumping to 0x{0:X4}",
                ((ushort) (memory[programCounter + 2] << 8 | memory[programCounter + 1])));
            programCounter =
                (ushort) (memory[programCounter + 2] << 8 | memory[programCounter + 1]); // memory[reg_h << 8 | reg_l]
            jumped = true;
        }
        ...
        public void inr(byte instruction)
        {
            switch (instruction & 0b00111000)
            {
                case 0b111000:
        ...
        public void dcr(byte instruction)
        {
            switch (instruction & 0b00111000)
            {
                case 0b111000:
                ...
                case 0b110000:
                    //Memory reference from H:L
                    memory[reg_h << 8 | reg_l]--;
                    flagCheck(memory[reg_h << 8 | reg_l]);
                    break;
            }
        }

        private void flagCheck(byte num)
        {
            setSF(checkIsMinus(num));
            setZF(num == 0b0);
            setPF(hasParity(num));
        }
